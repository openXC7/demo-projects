...
class _CRG(LiteXModule):
    def __init__(self, platform, sys_clk_freq, with_ddr=True):
        self.rst    = Signal()
        ...
        if with_ddr:
            # DDR3 SDRAM clocks (sys/sys2x/sys4x/sys4x_dqs/idelay) from a single PLL.
        ...
        elif sys_clk_freq == 100e6:
            # Minimal CRG (no DDR3, no PLL): buffer the 100MHz input clock directly.
            clk100     = platform.request("clk100")
            clk100_buf = Signal()
            self.specials += Instance("IBUF", i_I=clk100, o_O=clk100_buf)
            self.specials += Instance("BUFG", i_I=clk100_buf, o_O=self.cd_sys.clk)
            self.specials += AsyncResetSynchronizer(self.cd_sys, ~platform.request("cpu_reset_n") | self.rst)
        else:
            # Minimal CRG (no DDR3) at a non-100MHz sys clock: single-clkout PLL.
            self.pll = pll = S7PLL(speedgrade=-1)
            self.comb += pll.reset.eq(~platform.request("cpu_reset_n") | self.rst)
            pll.register_clkin(platform.request("clk100"), 100e6)
            pll.create_clkout(self.cd_sys, sys_clk_freq)
...
class BaseSoC(SoCCore):
    def __init__(self, variant="s7-50", sys_clk_freq=100e6,
        ...
        **kwargs):
        ...
        # CRG --------------------------------------------------------------------------------------
        self.crg = _CRG(platform, sys_clk_freq, with_ddr=not kwargs.get("integrated_main_ram_size", 0))
