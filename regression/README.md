...
| `fdse-fdpe-undefined-init` | #179 | **disabled — expected-red until #179 lands in nextpnr-xilinx main.** an FDSE/FDPE whose INIT parameter is *present but undefined* (`x`) was read as INIT=0 — `int_or_default` only falls back to the type default when the key is absent — so the FF got a ZINI feature and powered up 0 on silicon instead of the primitive's INIT=1 default (a Vivado netlist omits the parameter and was already handled; a yosys netlist leaves `INIT=x`) |
The default run executes **6** cases (all table rows except the one marked disabled). `dsp-const-only-pins` (#159) and `fdse-fdpe-undefined-init` (#179) remain disabled — expected-red until their fixes land in `nextpnr-xilinx` main; pass them explicitly (`run.sh dsp-const-only-pins`, `run.sh fdse-fdpe-undefined-init`) to run them, and re-enable them in the default list once merged.
