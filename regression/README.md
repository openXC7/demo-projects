...
The default run executes **10** cases — every row above except `dsp-const-only-pins` (#159), which is expected-red until that fix lands in `nextpnr-xilinx` main; run it explicitly with `run.sh dsp-const-only-pins` and re-enable it in the default list once merged. `fdse-fdpe-undefined-init` (#179), `const-holdout` (#184), `lutram-ram64x1s` (#195, fixed by #196) and `lutram-clkinv` (#201) all landed in main and are enabled.
