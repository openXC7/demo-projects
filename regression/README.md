...
| `bufio-in-use` | #167 | **blocked — needs a chipdb from prjxray-db ≥ 77e52f10.** a placed BUFIO never emitted `BUFIO_Y*.IN_USE`, so the buffer was never switched on. The fix (packer + `write_bufio`) is in, but the pad→BUFIO `I2IOCLK` segbits only landed in the fork's prjxray-db 77e52f10 (`f2a469b8`); this tree's chipdb (ab1fc60c) has neither the `I2IOCLK` bits nor `BUFIO_Y*.IN_USE`, so the router rejects the path and the design cannot reach a FASM. Enable once the chipdb is regenerated |
The default run executes **14** cases — every row above except `dsp-const-only-pins` (#159), which is expected-red until that fix lands in `nextpnr-xilinx` main, and `bufio-in-use` (#167), which is blocked on a chipdb regenerated from prjxray-db ≥ 77e52f10; run each explicitly with `run.sh <case>` and re-enable it in the default list once unblocked. `fdse-fdpe-undefined-init` (#179), `const-holdout` (#184), `lutram-ram64x1s` (#195, fixed by #196), `lutram-clkinv` (#201), `srl-init` (#181), `xorigport-unknown-name` (`7cfd1e90`), `srl-wemux` (`ccfae5ae`) and `dup-package-pin` (#8) all landed and are enabled.
